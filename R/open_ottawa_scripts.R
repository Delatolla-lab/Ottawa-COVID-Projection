...
data_extract <- function(object, object_name) {
  dataframe <- object
  ...
  if (is.numeric(dataframe[[1]])) {
    ...
    if (grepl("^20-", dataframe[[1]]) == TRUE ||
        grepl("^21-", dataframe[[1]]) == TRUE) {
    ...
    else if (grepl("2020-", dataframe[[1]]) == TRUE ||
             grepl("2021-", dataframe[[1]]) == TRUE) {
      dataframe[[1]] <- dataframe[[1]] %>%
        as.Date()
    }
  }
  ...
  # Check if every day is present from start to end
  first_date <- dataframe$Date[[1]]
  last_date <- dataframe$Date[[nrow(dataframe)]]
  num_of_days <- difftime(first_date, last_date, units = "days")
  if (nrow(dataframe)!=abs(as.numeric(num_of_days))+1) {
    stop(paste("Missing Dates found in",object_name))
  }
...
adjusted_function <- function(url) {
  ...
  if (grepl("^20-", adjusted_data[[1]]) == TRUE ||
      grepl("^21-", adjusted_data[[1]]) == TRUE) {
...
test_prep <- function(ottawa_test_data) {
  ottawa_test <- data_extract(ottawa_test_data, "ottawa_test")
  ottawa_test %>%
    select(-OBJECTID) %>%
    rename(
      date = "Date",
...
case_prep <- function(ottawa_case_data, ottawa_test) {
  ottawa_data <- data_extract(ottawa_case_data, "ottawa_case_data")
  ottawa_data %>%
    replace(is.na(.), 0) %>%
    rename(
      date = "Date",
...
data_creation <- function(ottawa_case_data, ottawa_test_data) {
  ...
  #ottawa_data <- ottawa_test

  # Cant run without cases
