covid_visualization <- function(data, parameter){
  ...
  trace1 <- list(
    ...
    y = data[,grepl(paste(as.character(parameter), 50, sep = "_"), names(data))]
  )
  trace2 <- list(
    fill = "tozeroy", 
    meta = list(columnNames = list(
    ...
    y = data[,grepl(paste(as.character(parameter), 60, sep = "_"), names(data))], 
    stackgroup = NULL
  )
  trace3 <- list(
    fill = "tozeroy", 
    ...
    y = data[,grepl(paste(as.character(parameter), 70, sep = "_"), names(data))], 
    stackgroup = NULL
  )
  trace4 <- list(
    meta = list(columnNames = list(
  ...
  layout <- list(
    ...
    yaxis = list(
      ...
      title = list(text = "Census (# of patients occupying beds)"), 
      autorange = TRUE
    ), 
    autosize = TRUE
  )
