...
## <b>Forecasting hospital cases using wastewater signal and vaccination data using Linear Regression model and Multivariate Adaptive Regression Spline (MARS) model.</b>
...
The wastewater signal and the vaccination data in the 5 year old shows the highest correlation with hospitalization as can be seen in the correlation plot below. To improve the correlation with other vaccination data, log transformation of the data can be tried.
...
```{r echo=FALSE, warning=FALSE, message = FALSE}
set.seed(101)
final_data <- final_data %>% select(-c(date, ObjectId))
colnames(final_data)
...
train.predictions <- (train.predictions)
train_results <- cbind(train.predictions, (train$observed_census_ICU_p_acute_care))
...
hospital.predictions <- (hospital.predictions)
results <- cbind(hospital.predictions, (test$observed_census_ICU_p_acute_care))
colnames(results) <- c('predicted', 'actual')
results <- as.data.frame(results)
print(head(results))
```

The RMSE of about 18.9 and MAPE of ~32% shows a decent model but not of very high accuracy.

Root Mean Squared Error for the Simple Linear Regression Model on the test data:
...
```{r echo=FALSE, warning=FALSE, message = FALSE}
sd(final_data$observed_census_ICU_p_acute_care)
```
...
ggplotly(linear_reg_plot) %>%
  layout(title = list(text = "<b>Comparison plot between Actual and predicted values for Linear Regression model on test data</b>", font = t), xaxis = list(title = ""), yaxis = list(title = "<b>Hospital Cases</b>"), width=1100)
...
The model has selected 8 predictor variables with 3 degree of interaction terms. The final model has 16 terms which includes the intercept. The Rsquare is 98.5%. The RMSE error in the model is 4.28. Therefore, a very high percentage of variance in the response is being explained by the predictor variables in the model. 

For the simple linear regression model the Rsquare was 87.0% and only explains 87% of variance in the response. The RMSE was 18.9.
...
results <- cbind(mars_predict$y, (test$observed_census_ICU_p_acute_care))
...
train_results <- cbind(mars_train_predict$y, (train$observed_census_ICU_p_acute_care))
train_results <- as.data.frame(train_results)
colnames(train_results) <- c('predicted', 'actual')
```
...
ggplotly(mars_plot) %>%
  layout(title = list(text = "<b>Comparison plot between Actual and predicted values for MARS model</b>", font = t), xaxis = list(title = ""), yaxis = list(title = "<b>Hospital Cases</b>"), width=1100)
```
...
```{r echo=FALSE, warning=FALSE, message = FALSE}
res <- test$observed_census_ICU_p_acute_care - mars_predict$y
res <- as.data.frame(res)
ggplot(res, aes(res)) + geom_histogram(fill = 'blue', alpha=0.5)
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
plot(test$observed_census_ICU_p_acute_care - mars_predict$y, 
