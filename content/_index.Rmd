...
t<- as.tbl(peak_projections) %>% mutate(End = lag(peak_hospital_census),
                                    ...
                                    Diff=End-peak_hospital_census,
                                     Percent=paste(round(Diff/End*100,0),"%", sep = "")) %>%
  ...
  theme(panel.background = element_rect(fill = 'white'), plot.title = element_text(hjust = 0.5)) +
    ...
    geom_text(aes(x=xpos,y =  End-Diff/2, label=paste("",as.character(Percent), " reduction from\n", as.character(intervention), sep = ""), hjust=-0.06, vjust = -0.2))
t
```

Going from no prevention & control to 70% physical distancing results in a **99%** reduction in peak hospitalization census.
