...
## Observed COVID-19 incidence in long-term care facilities {#long-term-care-data}
...
ltc_excpec <- calc_expected_values_for_n_weeks(test, number_weeks = 3, observed_columns_name = "incidence", first_day = 5, date_column = "Date")

ltc_excpec[[1]]$DT1 <- ltc_excpec[[3]][[1]]$expected_val
ltc_excpec[[1]]$DT2 <- ltc_excpec[[3]][[2]]$expected_val
ltc_excpec[[1]]$DT3 <- ltc_excpec[[3]][[3]]$expected_val
...
ltc_plot <- reworked_figure(
  xaxis = "Date",
  yaxis = list(RH_list, LTCH_list, Group_home_list,DT1, DT2, DT3),
  titles = c(y = "Daily Observed cases", x = "Date", title = "Long Term Care"),
  data = ltc_excpec[[1]]
)

doubling_time <- ltc_excpec[[2]]
expected_values <- ltc_excpec[[3]]
