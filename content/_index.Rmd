...
> **Halving time refers** to the time it takes for the number of hospitalized patients to halve in number.

In the figure, doubling/halving time was calculated for each 7 day period (Mon-Sun). Halving time is the time it takes to half the number of hospitalized patients.
...
- `r round(current_intervention, -1)` patients in `r paste(current_intervention_date)`, based on current physical distancing effectiveness, or a `r round(((no_intervention - current_intervention)/no_intervention*100), 0)`% reduction in peak hospitalizations from no prevention & control.

> **Hospital census `r Dt_label` of `r round(abs(exp_census[[2]][[3]]), 1)` days translates to an effective physical distancing of approximately `r round(effective_pd, 0)`%.** 
