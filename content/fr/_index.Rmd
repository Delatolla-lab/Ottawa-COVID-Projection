...
day_new_f <- which(ott_observed_episodes$date == ymd("2020-05-19")) # Date of phase 1 reopening
f_seg[seq(day_new_f, length(f_seg))] <- 2
day_ch <- which(ott_observed_episodes$date == ymd("2020-06-12")) # Date of phase 2 reopening
...
first_day <- min(ott_observed_episodes$date) # first observation date
...
ott_case_projections <- proj_generation(fit, lut, ott_observed_episodes, project_to = "2020-12-31", day_start_reduction = 7, pct_change = 10)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Prepare projection datasets for visualization
# Load datasets
...
hosp_admits <- c(ott_observed$observed_new_ICU_p_acute_care, rep(NA, (nrow(hosp_proj) - nrow(ott_observed))))
hosp_census <- c(ott_observed$observed_census_ICU_p_acute_care, rep(NA, (nrow(hosp_proj) - nrow(ott_observed))))
...
case_projection_plot(projection_data, ott_observed_episodes, current_col = current_case_hex, reduction_col = reduction_hex, increase_col = increase_hex, value_column = "observed_new_episodes", date_column = "date", title = "Projected daily COVID-19 cases in Ottawa", pct_change = 10)
```
