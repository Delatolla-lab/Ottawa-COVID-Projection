---
title: "Surveillance de la COVID-19 dans les eaux usées à Ottawa"
...
# libraries
# Disable scientific notation for numeric output in current session.
options(scipen = 999)
library(plotly)
library(tidyverse)
library(jsonlite)
...
variant_data <- ww_long_prep(ott_long_ww_data) %>%
  ...
  mutate(
    propB117 = propB117 * 100,
    sd = sd * 100
  ) %>%
  filter(!is.na(sd)) %>%
  mutate(propB117_7day = rollapply(propB117,
    width = 7,
    FUN = function(x) mean(x, na.rm = TRUE),
    by = 1, by.column = TRUE, partial = FALSE,
    fill = NA, align = "center"
  )) %>%
  mutate(sd_7day = rollapply(sd,
    width = 7,
    FUN = function(x) mean(x, na.rm = TRUE),
    by = 1, by.column = TRUE, partial = FALSE,
    fill = NA, align = "center"
...
wwintro <- insert_translation(localization_data, lang, "WWIntro", format_datetime_fr(time))
...
wwcollectdate <- insert_translation(localization_data, lang, "WWCollectDate", format_datetime_fr(last(ww_covid_waste$date), date_only=TRUE))
wwanalysesdate <- insert_translation(localization_data, lang, "WWAnalysesDate", format_datetime_fr(curr_day, date_only=TRUE))
```
...
sheet <- variant_sheet %>%
  select(
    "ENT_ID", "date_collected", "date_received_nml", "report_date", "turnaround_days",
