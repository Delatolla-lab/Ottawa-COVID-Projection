...
staff_cov_data<-load("../../Data/Observed data/cumulative_covid_absenteeism.RData")
...
#Loading health care worker staff two week Covid projections.
staff_projections <- load(file.path(getwd(), "../../Data/Historic Projections/historic_cases_projections_twoweekCovidEmployeeData.RData"))

# Transforming the Covid projections of the health care workers and the confidence intervals into rate per 1000 population
staff_cov_projections<- cases_forecast[[1]] %>% filter(variable == "reported_cases") %>% mutate(median = (median/7019)*1000)
staff_cov_projections<- cases_forecast[[1]] %>% filter(variable == "reported_cases") %>% mutate(upper_75 = (upper_75/7019)*1000)
staff_cov_projections<- cases_forecast[[1]] %>% filter(variable == "reported_cases") %>% mutate(lower_75 = (lower_75/7019)*1000)

# Transforming the Rt projections of the health care workers and the confidence intervals into rate per 1000 population
staff_rt_projection<- cases_forecast[[1]] %>% filter(variable == "R") %>% mutate(median = (median/7019)*1000)
staff_rt_projection<- cases_forecast[[1]] %>% filter(variable == "R") %>% mutate(upper_75 = (upper_75/7019)*1000)
staff_rt_projection<- cases_forecast[[1]] %>% filter(variable == "R") %>% mutate(lower_75 = (lower_75/7019)*1000)
...
staff_projections_text <- insert_translation(localization_data, lang, "hosp_projections_text")
...
`r observed_staff_incident`

```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Visualize current daily cases
source("../../R/observed_data.R")
observed_new_episodes <- list(type = "avg_data", y_column = "cummcovidPosAbs_pop", name = "Cumulative health care worker cases by episode date", color = rolling_avg_col, showlegend = TRUE, width = 4)

reworked_figure(
    xaxis = "date",
    yaxis = list(observed_new_episodes),
    titles = c(y = "Cumulative health care worker cases (per 1000 people)", x = "Date", title = "Cumulative incidence of COVID-19 positive staff", ticklabels = "%b %y"),
    data = ott_observed_staff, date_constraint = TRUE, constraint_val = constrain_val
  )
```

`r observed_staff_incident_cases_text`
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# visualize epinow forecast
short_term_plot(projections = hosp_proj, obs_data = ott_observed, obs_column = "observed_census_ICU_p_acute_care", forecast_type = "reported_cases", ylab = "Hospital census", title = "Short-term hospital census projections in Ottawa", tick_labels_date = "%b %y", tick_period = "1 month")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# visualize epinow forecast
short_term_plot(projections = staff_cov_projections, obs_data = covid_data, obs_column = "covidPosAbs_pop", forecast_type = "reported_cases", ylab = "Health care worker cases (per 1000 people)", title = "Short-term COVID-19 positive staff projection in Ottawa", tick_labels_date = "%b %y", tick_period = "1 month")
```
`r line_break`
...
source("../../R/epinow_functions.R")
# visualize epinow forecast
short_term_plot(projections = staff_rt_projection, obs_data = covid_data, obs_column = "covidPosAbs_pop", forecast_type = "R", ylab = "Reproduction number of Health care worker cases (per 1000 people)", title = "Short-term COVID-19 positive staff projection in Ottawa", tick_labels_date = "%b %y", tick_period = "1 month")
```
