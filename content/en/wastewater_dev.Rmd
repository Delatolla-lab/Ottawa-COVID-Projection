...
delta_data <- ww_long_prep(ott_long_ww_data) %>%
  select(date, aggregation, var_delta) %>%
  filter(!is.na(var_delta)) %>%
  spread(aggregation, var_delta) %>%
  rename(prop_delta = "single",
         sd_delta = "sd") %>%
  mutate(prop_delta = prop_delta*100,
         sd_delta = sd_delta*100) %>%
  filter(!is.na(sd_delta)) %>%
  mutate(prop_delta_3day = rollapply(prop_delta, width=3,
                  FUN=function(x) mean(x, na.rm = TRUE),
                  by=1, by.column=TRUE, partial=FALSE,
                  fill=NA, align="center")) %>%
  mutate(sd_delta_3day = rollapply(sd_delta, width=3,
                  FUN=function(x) mean(x, na.rm = TRUE),
                  by=1, by.column=TRUE, partial=FALSE,
                  fill=NA, align="center")) %>%
  drop_na()

omicron_data <- ww_long_prep(ott_long_ww_data) %>%
  select(date, aggregation, varC2811T) %>%
  filter(!is.na(varC2811T)) %>%
  spread(aggregation, varC2811T) %>%
  rename(prop_omicron = "single",
         sd_omicron = "sd") %>%
  mutate(prop_omicron = prop_omicron*100,
         sd_omicron = sd_omicron*100) %>%
  filter(!is.na(sd_omicron)) %>%
  mutate(prop_omicron_3day = rollapply(prop_omicron, width=3,
                  FUN=function(x) mean(x, na.rm = TRUE),
                  by=1, by.column=TRUE, partial=FALSE,
                  fill=NA, align="center")) %>%
  mutate(sd_omicron_3day = rollapply(sd_omicron, width=3,
                  FUN=function(x) mean(x, na.rm = TRUE),
                  by=1, by.column=TRUE, partial=FALSE,
                  fill=NA, align="center")) %>%
  drop_na()

delta_omicron_data <- delta_data %>%
  full_join(omicron_data, by = "date") %>%
  arrange(date)
...
delta_col <- "rgb(226, 127, 88)"
delta_shade <- "rgba(226, 127, 88, 0.2)"
omicron_col <- "rgb(102, 51, 153)"
omicron_shade <- "rgba(102, 51, 153, 0.2)"
```
...
delta_call <- list(type = "signal_data", y_column = "prop_delta", name = "% Delta RNA", color = delta_col)

delta_3_day_avg <- list(type = "avg_data", y_column = "prop_delta_3day", name = "Delta midpoint rolling average", color = delta_col, width = 3)

omicron_call <- list(type = "signal_data", y_column = "prop_omicron", name = "% Omicron RNA", color = omicron_col)

omicron_3_day_avg <- list(type = "avg_data", y_column = "prop_omicron_3day", name = "Omicron midpoint rolling average", color = omicron_col, width = 3)
```
...
Data last reported `r as.character(tail(waste_clean$reportDate, n=1))`.
...
The next plot illustrates the 7-day midpoint mean in daily covid-19 wastewater viral signal. This number is the average of a week’s readings; today's reading, the previous 3 days, and the subsequent 3 days. Also on the graph are various comparators (e.g. reported daily covid-19 cases, hospitalization count), which can be individually selected by toggling the menu on the right.
...
reworked_figure(xaxis = "date", yaxis = list(delta_call, delta_3_day_avg), error_bands = TRUE, error_pct = TRUE, error_data = "sd_delta_3day", error_col = delta_shade, titles = c(y = "Proportion (%)", y2 = "", x = "Date", title = "<b>Proportion of variant Delta RNA signal in wastewater, Ottawa</b>"), data = delta_data, ticks = FALSE)
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.asp = 0.75}
reworked_figure(xaxis = "date", yaxis = list(omicron_call, omicron_3_day_avg), error_bands = TRUE, error_pct = TRUE, error_data = "sd_omicron_3day", error_col = omicron_shade, titles = c(y = "Proportion (%)", y2 = "", x = "Date", title = "<b>Proportion of variant Omicron RNA signal in wastewater, Ottawa</b>"), data = omicron_data, ticks = FALSE)
```
