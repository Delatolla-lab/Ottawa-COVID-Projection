...
drive_download(as_id("1jUAWx9_r7CLhh5vtRNNjCRDMX4nEKEx5gvXM_tpsxhE"), path = "../../Data/ww_variant_text.csv", overwrite = TRUE)

variant_sheet <- read.csv(file.path(getwd(), "../../Data/ww_variant_text.csv"))

names(variant_sheet) <- c("ENT_ID", "date_collected", "date_received_nml", "report_date", "turnaround_days",
                         "CDC_N1", "CDC_N2", "method", "sequencing_platform", "breadth_of_coverage_pct",
                         "mean_coverage_depth", "pangolin_lineage", "detect", "consensus_subconsensus_NA",
                         "prop_voc_mutations", "frac_voc_detected")

variant_text <- variant_df_prep(variant_sheet)
...
alphaSummary <- ifelse(alphaDetect == TRUE, paste("Alpha (B.1.1.7) detected in", alphaLevel, "levels, last reported on", paste0(date, ".")), paste("Alpha (B.1.1.7) not detected, last reported on", paste0(date, ".")))
...
deltaSummary <- ifelse(deltaDetect == TRUE, paste("Delta (B.1.617+) detected in", deltaLevel, "levels, last reported on", paste0(date, ".")), paste("Delta (B.1.617+) not detected, last reported on", paste0(date, ".")))
...
See plot below, “Proportion of variant, Alpha (B.1.1.7) RNA in wastewater, Ottawa”.
