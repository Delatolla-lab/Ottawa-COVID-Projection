...
The figures below were created by researchers at CHEO RI and uOttawa. Learn more about wastewater epidemiology and its role in covid-19 surveillance on Ottawa Public Health's [website](https://www.ottawapublichealth.ca/en/reports-research-and-statistics/statistics-on-coronavirus-19-covid-19-in-ottawa.aspx). 
...
new_case_5_day_call <- list(type = "observed_data", y_column = "observed_new_cases_5_day", name = "5 day mean \nin reported new cases", color = case_col, opacity = 0.15)

new_case_10_day_call <- list(type = "observed_data", y_column = "observed_new_cases_10_day", name = "10 day mean \nin reported new cases", color = case_col, opacity = 0.15)

new_case_7_day_call <- list(type = "observed_data", y_column = "observed_new_cases_7_day", name = "7 day mean \nin reported new cases", color = case_col, opacity = 0.15)
...
n1_n2_10_day_call <- list(type = "avg_data", y_column = "N1_N2_10_day", name = "10-day mean viral signal", color = rolling_avg_col, yaxis = "y2")

n1_n2_5_day_call <- list(type = "avg_data", y_column = "N1_N2_5_day", name = "5-day mean viral signal", color = rolling_avg_col, yaxis = "y2")

n1_n2_7_day_call <- list(type = "avg_data", y_column = "N1_N2_7_day", name = "7-day mean viral signal", color = rolling_avg_col, yaxis = "y2")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
reworked_figure(xaxis = "date", yaxis = list(n1_n2_7_day_call), yaxis2 = list(new_community_case_7_day_call), titles = c(y = "Normalized viral copies*", y2 = "# of reported new cases", x = "Date", title = "Average COVID-19 viral signal \nin wastewater with respect to new community cases"), data = ott_covid_waste)
```
* Viral copies data is normalized using a seasonally stable fecal biomarker; pepper mild mottle virus (PMMoV). See methods for more details.

```{r, echo=FALSE, warning=FALSE, message=FALSE}
reworked_figure(xaxis = "date", yaxis = list(n1_n2_10_day_call), yaxis2 = list(new_community_case_10_day_call), titles = c(y = "Normalized viral copies*", y2 = "# of active cases", x = "Date", title = "Average COVID-19 viral signal \nin wastewater with respect to new community cases"), data = ott_covid_waste)
```
* Viral copies data is normalized using a seasonally stable fecal biomarker; pepper mild mottle virus (PMMoV). See methods for more details.

```{r, echo=FALSE, warning=FALSE, message=FALSE}
reworked_figure(xaxis = "date", yaxis = list(n1_n2_7_day_call), yaxis2 = list(new_community_case_10_day_call), titles = c(y = "Normalized viral copies*", y2 = "# of active cases", x = "Date", title = "Average COVID-19 viral signal \nin wastewater with respect to new community cases"), data = ott_covid_waste)
```
* Viral copies data is normalized using a seasonally stable fecal biomarker; pepper mild mottle virus (PMMoV). See methods for more details.

```{r, echo=FALSE, warning=FALSE, message=FALSE}
reworked_figure(xaxis = "date", yaxis = list(n1_n2_10_day_call), yaxis2 = list(new_community_case_7_day_call), titles = c(y = "Percentage change (%)", y2 = "# of reported new cases", x = "Date", title = "Average COVID-19 viral signal \nin wastewater with respect to new community cases"), data = ott_covid_waste)
```