...
#covid_data <- file.path(getwd(), "../../Data/Observed data/DailyPos.xlsx")
#staff_cov_data <- read_excel(covid_data, sheet = "Data")
#staff_cov_data <- staff_cov_data %>% mutate(date = as.Date(sampleDate)) %>% select(-c(sampleDate))

load("../../Data/Observed data/cumulative_data_absenteeism.RData")
staff_cov_data <- covid_data
# data
...
ott_covid_waste <- ott_covid_waste %>% full_join(staff_cov_data, by = "date")
...
staff_incident_cases_7_day <- list(type = "observed_data", y_column = "covidPosAbs_pop", name = "Observed COVID-19 cases in health care workers in Ottawa by episode date", short_name = "Health care worker cases", color = case_col, opacity = 0.15)
...
n1_n2_10_day_call <- list(type = "avg_data", y_column = "N1_N2_10_day", name = "10-day rolling mean viral signal", short_name = "10-day", color = rolling_avg_col, yaxis = "y2", width = 4)

n1_n2_5_day_call <- list(type = "avg_data", y_column = "N1_N2_5_day", name = "5-day rolling mean viral signal", short_name = "5-day", color = rolling_avg_col, yaxis = "y2", width = 4)

n1_n2_7_day_call <- list(type = "avg_data", y_column = "N1_N2_7_day", name = "7-day rolling mean viral signal", short_name = "7-day", color = rolling_avg_col, yaxis = "y2", width = 4)
...
A method modification was applied on June 8th 2021 to the Ottawa wastewater data set. The magnitudes (not the shape of the curve) from June 8th to present have been retroactively modified on April 12, 2022 to better align the modified method to previous data. The two methods are the same method, with the new method being one that is adapted for settings of lower wastewater detection by processing slightly larger amounts of wastewater. This means that had this new method been applied to the old samples, the amount of virus detected would be proportionally larger. 

It is important to remember that the exact meaning of the magnitude of the viral signal is not fully understood, particularly as it is affected by numerous known and unknown factors (eg. the variant in current circulation, the level of immunization in the population, etc.). The most valuable indicator that wastewater follows is actually the relative increase and decrease in overall signal, which tends to precede increases and decreases in hospitalization.
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
reworked_figure(xaxis = "date", yaxis = list(daily_viral_signal_call, daily_viral_signal_call_omit, n1_n2_7_day_call), titles = c(y = "Normalized viral copies*", y2 = "", x = "Date", title = "<b>COVID-19 wastewater viral signal, Ottawa</b>"), data = ott_covid_waste, date_constraint = TRUE, ticks = FALSE, constraint_val = 40, ticklabels = "%b %d")
```
...
reworked_figure(xaxis = "date", yaxis = list(n1_n2_7_day_call), yaxis2 = list(hosp_call, new_case_7_day_call, new_episode_7_day_call, active_case_call, pct_positivity_call_7_day, staff_incident_cases_7_day), titles = c(y = "Normalized viral copies*", y2 = "", x = "Date", title = "<b>7-day mean COVID-19 wastewater viral signal, Ottawa</b>"), data = ott_covid_waste, yaxis2_button = TRUE, y2_button_name = "Comparator", height = 300, date_constraint = TRUE, constraint_val= 301)
```
...
<img src="/images/voc_image_22April22.png" width = "90%" height = "90%"/>
...
\* Percent positivity refers to the percentage of individuals tested for COVID-19 infection in a given day that test positive.
