...
ww_data = ww_forecast_jul_pres[[1]] %>%
  filter(variable == "reported_cases") %>%
  rename_with(function(x) paste(x, "_ww"), .cols = -date)
hosp_data = hosp_forecast_jul_pres[[1]] %>%
  filter(variable == "reported_cases") %>%
  rename_with(function(x) paste(x, "_hosp"), .cols = -date)
cases_data = cases_forecast_jul20_pres22[[1]] %>%
  filter(variable == "reported_cases") %>%
  rename_with(function(x) paste(x, "_cases"), .cols = -date)
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
save(final_merged_data, file = "merged_data_cases_hosp_ww.RData")
```
...
joined_with_observed_cases = ww_clean %>% select(date, N1_N2_avg_clean) %>% mutate(date = as.Date(date)) %>% inner_join(joined_with_observed_cases, by = "date") %>% relocate(N1_N2_avg_clean, .after = median_ww)
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
save(joined_with_observed_cases, file = "merged_data_cases_hosp_ww_observed.RData")
```
